...
def prepare_mask_and_masked_image(image, mask, height, width, return_image: bool = False):
    ...
    if image.shape[1] == 4:
        ...
        hand_image = image * (mask >= 0.5)

    # n.b. ensure backwards compatibility as old function does not return image
    if return_image:
        return mask, masked_image, image, hand_image
...
class StableDiffusionXLControlNetInpaintPipeline(DiffusionPipeline, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def prepare_mask_latents(
            self, mask, masked_image, hand_image, batch_size, height, width, dtype, device, generator, do_classifier_free_guidance
    ):
        ...
        if hand_image is not None:
            hand_image = hand_image.to(device=device, dtype=dtype)
            hand_image_latents = self._encode_vae_image(hand_image, generator=generator)
            if hand_image_latents.shape[0] < batch_size:
                if not batch_size % hand_image_latents.shape[0] == 0:
                    raise ValueError(
                        "The passed images and the required batch size don't match. Images are supposed to be duplicated"
                        f" to a total batch size of {batch_size}, but {masked_image_latents.shape[0]} images were passed."
                        " Make sure the number of images that you pass is divisible by the total requested batch size."
                    )
                hand_image_latents = hand_image_latents.repeat(
                    batch_size // hand_image_latents.shape[0], 1, 1, 1
                )

            hand_image_latents = (
                torch.cat([hand_image_latents] * 2) if do_classifier_free_guidance else hand_image_latents
            )

            # aligning device to prevent device errors when concating it with the latent model input
            hand_image_latents = hand_image_latents.to(device=device, dtype=dtype)

        return mask, masked_image_latents, hand_image_latents
    ...
    def __call__(
        ...
        mask, masked_image, init_image, hand_image = prepare_mask_and_masked_image(
        ...
        # hand_latents_outputs = self.prepare_latents(
        #     batch_size * num_images_per_prompt,
        #     num_channels_latents,
        #     height,
        #     width,
        ...
        #     latents1, noise1, hand_latents = hand_latents_outputs
        else:
            latents, noise = latents_outputs

        # 7. Prepare mask latent variables
        mask, masked_image_latents, hand_image_latents = self.prepare_mask_latents(
            mask,
            masked_image,
            hand_image,
            batch_size * num_images_per_prompt,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print(control_model_input)
                # import pdb;pdb.set_trace()
                if MOE:
                    gating_network.cuda()
                    cond_scale = gating_network(hand_image_latents.to(torch.float32)).detach().cpu().numpy()[0]
                    # print(cond_scale)
